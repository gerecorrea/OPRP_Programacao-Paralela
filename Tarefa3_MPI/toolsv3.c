...
int fileout_matrizv2 (matriz_bloco_t *submatrizA, matriz_bloco_t *submatrizB, matriz_bloco_t *submatrizC, FILE *infile){
  int lin = submatrizA->bloco->lin_fim - submatrizA->bloco->lin_inicio + 1;
  int col = submatrizB->bloco->col_fim - submatrizB->bloco->col_inicio + 1;
  ...
  for (int i=submatrizA->bloco->lin_inicio; i <= submatrizA->bloco->lin_fim; i++) {
    for (int j=submatrizB->bloco->col_inicio; j <= submatrizB->bloco->col_fim; j++){
      fprintf(infile,"%d,", submatrizC->matriz[i][j]);
    }
    fprintf(infile,"\n");
  }
  return 0;
