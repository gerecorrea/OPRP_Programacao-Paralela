...
// // Multiplicação normal com OpenMP - Tudo ok - tempos aleatórios às vezes
		...
		//Caso deixasse o start_time aqui os tempos obtidos são mto altos e sem sentido. Não sei o motivo.
			//Mas não condiz com a realidade pois o código é instantâneo em sua execução
		//Deixando dentro isso é mais raro, mas ainda retorna um número gigante sem sentido as vezes (desconsiderar esses casos)
		//Possivelmente é porque na prática o número que se encotnra calculado seja mto pequeno?? Acho que não, mas não encontrei motivos. Não fui o único a ocorrer tal coisa.
		start_time = wtime();
		...
		end_time = wtime();
		#ifndef DEBUG
		float mediaNormalOpenMP;
		printf ("media tempo: %.2lf", end_time-start_time);
		if(ntasks<=6)
			mediaNormalOpenMP = (end_time-start_time)/tamLoop;
		else
			mediaNormalOpenMP = (end_time-start_time)/tamLoop;
		#endif
...
// // Multiplicação MultiThreads em Bloco com OpenMP:
			...
			for(int i=0; i<tamLoop; i++){
				...
				if(tid < 6){
					for(int ii=vet[tid]; ii<vet[tid+1]; ii++){ 
						...
						//Obs: nessa dunção, em matriz-operacoesv3.c, realizo todo o processo de submatrizes
		...
		end_time = wtime();
		#ifndef DEBUG
		float mediaBlocoOpenMP;
		if(ntasks<=6)
			mediaBlocoOpenMP = (end_time-start_time)/tamLoop;
		else
			mediaBlocoOpenMP = (end_time-start_time)/tamLoop;
		#endif
