...
int main(int argc, char *argv[]) {
	...
	for (int ii=0; ii < 2; ii++) {
		...
		start_time = wtime();
		mat_soma[ii] = msomar(&mat_a, &mat_b, ii);
		end_time = wtime();
	...
	// Tarefa 2 - multiplicação de submatrizes - começa aqui:
	// %%%%%%%%%%%%%%%%%%%%%%%%% BEGIN %%%%%%%%%%%
	int divisor = 3; //qtd de cortes
	mat_multBloco = (mymatriz **) malloc (sizeof(mymatriz *));
	start_time = wtime();
	mat_multBloco[0] = opera_submatrizes(&mat_a, &mat_b, divisor);
	end_time = wtime();
	printf("\n\tRuntime: %f\n\n", end_time - start_time);
	printf("------- Fim multiplicação de submatrizes -----------\n\n");
	
	//COMPARAÇÕES DA EM BLOCO COM OUTRAS.
	printf ("Matriz gerado por multiplicação em bloco (main):\n");
	mimprimir(*mat_multBloco);
	printf ("\nComparando uma das normais com a em bloco:\n");
	mcomparar(*mat_multBloco, mat_mult[0]);
	// %%%%%%%%%%%%%%%%%%%%%%%%% END %%%%%%%%%%%%%%%%%
